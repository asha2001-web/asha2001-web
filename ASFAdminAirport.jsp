...
<%	
	...
	dbaccess.makeConnection("DASHBOARD");//Hyderabad.
	...
	strQuery = "select x.EXPIRE_DT,x.USER_STATUS,x.USER_CD,nvl(x.ACCESS_LEVEL,'EMPTY ')ACCESS_LEVEL,x.PASSWORD, x.usrid,x.Name,x.primary_module,x.dept_cd,x.emp_cd,x.airport_cd, to_date(to_char(x.EXPIRE_DT,'dd/mm/yyyy'),'dd/mm/yyyy') - to_date(to_char(sysdate,'dd/mm/yyyy'),'dd/mm/yyyy') as tDate, handles_complaints,'Aviation Security Fee' description,'/view/tc/ATCMain.jsp' mainpage,terminal_id, nvl(L.ADDRESS,' ')ADDRESS,L.LOCATION_NAME, x.DIVISION_CD,usertheme, x.REGION_CD from AM_ASF_ADMIN_USER_MT x,TC_LOCATION_MT L where L.LOCATION_CD=x.airport_cd and x.USER_CD='"+usercd+"'"; 

	System.out.println("strQuery JVC...."+strQuery);
	...
	{	
		...
		{	
			...
			{   
				...
				session.setAttribute("region_cd",loginRs.getString("REGION_CD"));
            ...
            dbaccess.closeRs();
				...
				String access=sessionBean.getAccessLevel();
					...
					loginRs.close();
					con.close();
					response.sendRedirect(appContext+"/ASF/ASFAdminAims.jsp");
